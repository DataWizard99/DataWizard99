...
if st.button('Predict'):
    ...
    if data.empty:
        ...
        if x_data is not None:
            ...
            # Define train_data_len based on the available data
            train_data_len = len(data) - len(predictions)
            
            # Initialize 'Predictions' column with NaN
            data_for_plot['Predictions'] = np.nan

            # Update only the rows with predictions (match lengths)
            data_for_plot.iloc[-len(predictions):, data_for_plot.columns.get_loc('Predictions')] = predictions.flatten()
